...
public abstract class WOInput extends WOHTMLDynamicElement {
    ...
    (final String _name, final Map<String, WOAssociation> _assocs, final WOElement _template)
  {
    ...
    final WOAssociation value = grabAssociation(_assocs, "value");
  ...
  public void takeValuesFromRequest(final WORequest _rq, final WOContext _ctx) {
    ...
    final String formName  = elementNameInContext(_ctx);
    ...
    if (formValue == null) {
       ...
       * processed initially.
       */
      final int idx = formName.indexOf(':');
      if (idx > 0) {
        final String n = formName.substring(0, idx);
    ...
    try {
      formValue = parseFormValue(formValue, _ctx);
    }
    catch (final ParseException e) {
      if (handleParseException(formName, formValue, e, _ctx))
        return;
    }
  ...
  public void appendAttributesToDescription(final StringBuilder _d) {
    ...
    if (this.eid == this.name) {
      appendAssocToDescription(_d, "idname", this.eid);
    }
    else {
      appendAssocToDescription(_d, "id",    this.eid);
      appendAssocToDescription(_d, "name",  this.name);
    }

    if (this.readValue == this.writeValue)
      appendAssocToDescription(_d, "value", this.readValue);
    else {
      if (this.readValue != null)
        appendAssocToDescription(_d, "readValue", this.readValue);
      if (this.writeValue != null)
        appendAssocToDescription(_d, "writeValue", this.writeValue);
    }
  }
}
