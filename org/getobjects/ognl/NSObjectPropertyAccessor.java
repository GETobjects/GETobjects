...
/*
 * NSObjectPropertyAccessor
 * 
 * This differs to the KVC accessor because we can directly call NSObject KVC
 * methods w/o the Utility.
 * 
 * ... inspired by but not copied from WOOgnl ;-)
 * http://en.wikibooks.org
 *   /wiki/Programming:WebObjects/Project_WONDER/Frameworks/WOOgnl
 */
public class NSObjectPropertyAccessor implements PropertyAccessor {
    ...
    throws OgnlException
  {
    ...
    try {
      ...
      ((NSKeyValueCoding)_target).takeValueForKey(_value, key);
    }
    catch (Exception e) {
    ...
    throws OgnlException
  {
    ...
    try {
      /* hey, we even support other KVC objects, not just NSObject ;-) */
      return ((NSKeyValueCoding)_target).valueForKey((String)_name);
    }
    catch (Exception e) {
      throw new OgnlException(_name != null ? _name.toString() : null, e);
    }
